...
public class MasterAdminCreateOrder implements OrderCreateStrategy {
    ...
    public CreateOrderResponse createOrder(CreateOrderRequest createOrderRequest, Long currentUser, Role role) {
        ProductResponse productResponse = orderValidator.checkProductExist(createOrderRequest.getProductId());
        orderValidator.checkCompanyExist(createOrderRequest.getSupplierId(), "SUPPLIER");
        orderValidator.checkCompanyExist(createOrderRequest.getReceiverId(), "RECEIVER");
        OrderStatus orderStatus = orderValidator.validateStockAvailability(productResponse, createOrderRequest.getOrderQuantity());
